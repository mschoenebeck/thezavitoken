#include "thezavitoken.hpp"

thezavitoken::thezavitoken(name self, name code, datastream<const char *> ds) :
    contract(self, code, ds)
{
}

void thezavitoken::init(const uint64_t& depth)
{
    require_auth(_self);
...
// zMint
void thezavitoken::mint(const checksum256& epk_s,
...
// zTransfer
void thezavitoken::ztransfer(const checksum256& epk_s,
                             const vector<uint128_t>& ciphertext_s,
                             const checksum256& epk_r,
                             const vector<uint128_t>& ciphertext_r,
                             const string& proof,
...
// zBurn
void thezavitoken::burn(const checksum256& epk_s,
                        const vector<uint128_t>& ciphertext_s,
                        const checksum256& epk_r,
                        const vector<uint128_t>& ciphertext_r,
                        const string& proof,
...
void thezavitoken::create(const name& issuer, const asset& maximum_supply)
{
    require_auth(_self);
...
void thezavitoken::issue(const name& to, const asset& quantity, const string& memo)
{
...
void thezavitoken::retire(const asset& quantity, const string& memo)
{
    auto sym = quantity.symbol;
    check(sym.is_valid(), "invalid symbol name");
    check(memo.size() <= 256, "memo has more than 256 bytes");
...
void thezavitoken::transfer(const name& from, const name& to, const asset& quantity, const string& memo)
{
...
void thezavitoken::open(const name& owner, const symbol& symbol, const name& ram_payer)
{
...
void thezavitoken::close(const name& owner, const symbol& symbol)
{
    require_auth(owner);
    accounts acnts(get_self(), owner.value);
...
void thezavitoken::add_balance(const name& owner, const asset& value, const name& ram_payer)
{
...
void thezavitoken::sub_balance(const name& owner, const asset& value)
{
...
asset thezavitoken::get_supply(const symbol_code& sym) const
{
...
asset thezavitoken::get_balance(const name& owner, const symbol_code& sym) const
{
...
void thezavitoken::insert_into_merkle_tree(const checksum256& val, const bool& add_root_to_list)
{
    // fetch global stats
    gs_t gs(_self, _self.value);
    auto stats = gs.find(1);
...
bool thezavitoken::is_root_valid(const checksum256& root)
{
...
void thezavitoken::add_txdata_to_list(const uint8_t& type,
